...
export default function Terms(props) {
    return (
        <Grid container className='termParent' justifyContent={"center"} direction={'row'}>
            <Typography variant='h1' mt={3} mb={3} sm={12}>TERMS AND CONDITION</Typography>
                ...
                <Grid container direction={"column"} spacing={2} justifyContent='center' alignItems={"center"}>
                    {termsAndConditions.map((data, index) => (
                        <Grid container>
                            <Grid item sm={3}></Grid>
                            <Grid item sm={6}>
                                <Typography variant='h6'>{index + 1}. {data.heading}</Typography>
                                <Typography variant='body1' style={{ textAlign: "left" }}>--{data.description}</Typography>
                            </Grid>
                            <Grid item sm={3}></Grid>
                        </Grid>
                    ))}
